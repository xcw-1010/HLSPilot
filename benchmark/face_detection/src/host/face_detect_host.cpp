...
int main(int argc, char ** argv) 
{
    ...
    cl_program program = xcl_import_binary_file(world, "face_detect.hw.xclbin");
    cl_kernel krnl_face_detect = xcl_get_kernel(program, "face_detect");
    ...
    cl_mem devMemInData = xcl_malloc(world, CL_MEM_READ_ONLY, IMAGE_WIDTH * sizeof(unsigned char));
    ...
    for (int i = 0; i < IMAGE_HEIGHT; i++) {
        xcl_memcpy_to_device(world, devMemInData, Data[i], IMAGE_WIDTH * sizeof(unsigned char));
